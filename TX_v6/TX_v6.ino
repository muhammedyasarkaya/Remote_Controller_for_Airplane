...
*/
const byte cal_LED = 5;
uint32_t cal_millis = 0;
uint32_t cal_time_along = 1000000;
uint8_t throttle_MIN_cal = 255;
uint8_t throttle_MAX_cal = 0;
uint8_t aileron_MIN_cal = 255;
uint8_t aileron_MAX_cal = 0;
uint8_t elevator_MIN_cal = 255;
uint8_t elevator_MAX_cal = 0;
uint8_t rudder_MIN_cal = 255;
uint8_t rudder_MAX_cal = 0;
/*
//  MULTIPLIER VARIABLE
*/
...
void internal_interrupt(){     
    // MULTIPLIER MUST BE BETWEEN 0-1
...
*/
void calibration(){  
/*  First blink is for finding minimums.
//  Second blink is for finding maximums.
//  Features: Saving the values to the EEPROM
*/
    digitalWrite(cal_LED, HIGH);
    delay(1000);
    digitalWrite(cal_LED, LOW);
    uint32_t cal_millis = millis();
    while( cal_millis <= (millis() + cal_time_along) ){
        if( analogRead(A0) < throttle_MIN_cal ){
            throttle_MIN_cal = analogRead(A0);
        }
        if( analogRead(A1) < aileron_MIN_cal ){
            aileron_MIN_cal = analogRead(A1);
        }
        if( analogRead(A2) < elevator_MIN_cal ){
            elevator_MIN_cal = analogRead(A2);
        }
        if( analogRead(A3) < rudder_MIN_cal ){
            rudder_MIN_cal = analogRead(A3);
        }
    }
    digitalWrite(cal_LED, HIGH);
    delay(1000);
    ...
    cal_millis = millis();
    while( cal_millis <= (millis() + cal_time_along) ){
        if( analogRead(A0) > throttle_MAX_cal ){
            throttle_MAX_cal = analogRead(A0);
        }
        if( analogRead(A1) > aileron_MAX_cal ){
            aileron_MAX_cal = analogRead(A1);
        }
        if( analogRead(A2) > elevator_MAX_cal ){
            elevator_MAX_cal = analogRead(A2);
        }
        if( analogRead(A3) > rudder_MAX_cal ){
            rudder_MAX_cal = analogRead(A3);
        }
    }
    /*  
        Serial.println("CALIBRATION RESULTS (MAX-MIN)");
        Serial.println(String("throttle : ") + throttle_MIN_cal + String(" : ") + throttle_MAX_cal);
        Serial.println(String("aileron : ") + aileron_MIN_cal + String(" : ") + aileron_MAX_cal);
        Serial.println(String("elevator : ") + elevator_MIN_cal + String(" : ") + elevator_MAX_cal);
        Serial.println(String("rudder : ") + rudder_MIN_cal + String(" : ") + rudder_MAX_cal);
        Serial.println("--------------------------------------------------------");
    */
}
...
void loop(){
    ...
    throttle_input = map(analogRead(A0),throttle_MIN_cal, throttle_MAX_cal, throttle_MIN, throttle_MAX);
    aileron_input = map(analogRead(A1), aileron_MIN_cal, aileron_MAX_cal, aileron_MIN_angle, aileron_MAX_angle);
    elevator_input = map(analogRead(A2), elevator_MIN_cal, elevator_MAX_cal, elevator_MIN_angle, elevator_MAX_angle);
    rudder_input = map(analogRead(A3), rudder_MIN_cal, rudder_MAX_cal, rudder_MIN_angle, rudder_MAX_angle);
